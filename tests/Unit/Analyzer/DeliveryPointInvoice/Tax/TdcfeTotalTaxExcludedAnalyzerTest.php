...
use App\Analyzer\DeliveryPointInvoice\Tax\TdcfeTotalTaxExcludedAnalyzer;
 ...
 * @group analyzer
 * @group analyzer-tax
 * @group analyzer-tax-tdcfe
 */
class TdcfeTotalTaxExcludedAnalyzerTest extends WebTestCase
{
    public function testCanGetAnalyzerName()
    {
        ...
        $analyzer = new TdcfeTotalTaxExcludedAnalyzer($translationManager, $logger, $taxService, $consumptionService, $conversionService);

        $this->assertEquals('delivery_point_invoice.tax.tdcfe.total_tax_excluded', $analyzer->getName());
    }
    ...
    public function testCanThrowIgnoreEventIfWeHaveTaxInDeliveryPointInvoiceButWithoutAmount()
    {
        ...
        $tax = new InvoiceTax();
        $tax->setType(InvoiceTax::TYPE_TAX_TDCFE);
        ...
        $analyzer = $this->getAnalyzerMock(TdcfeTotalTaxExcludedAnalyzer::class, [$taxService, $consumptionService, $conversionService]);
        $analyzer->expects($this->once())->method('ignore')->with(transInfo('ht_amount_missing', ['type' => 'tax', 'tax' => strtoupper(InvoiceTax::TYPE_TAX_TDCFE)]), 'tax.tdcfe.total');
    ...
    public function testCanThrowAnomalyIfDataDoesNotMatch()
    {
        ...
        $analyzer->expects($this->once())->method('anomaly')->with(
            ...
            transInfo('amount_incorrect', [
                'amount_type' => 'HT',
                'type' => 'tax',
                'tax' => strtoupper(InvoiceTax::TYPE_TAX_TDCFE)
            ]),
            ...
            'tax.tdcfe.total', new AmountDiff(10**5, 11.11, Anomaly::PROFIT_PROVIDER)
        );
        $analyzer->analyze($deliveryPointInvoice);
    }
