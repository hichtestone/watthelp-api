...
use App\Analyzer\DeliveryPointInvoice\Tax\CspeTotalTaxExcludedAnalyzer;
 ...
 * @group analyzer
 * @group analyzer-tax
 * @group analyzer-tax-cspe
 */
class CspeTotalTaxExcludedAnalyzerTest extends WebTestCase
{
    public function testCanGetAnalyzerName()
    {
        ...
        $analyzer = new CspeTotalTaxExcludedAnalyzer($translationManager, $logger, $taxService, $consumptionService, $conversionService);

        $this->assertEquals('delivery_point_invoice.tax.cspe.total_tax_excluded', $analyzer->getName());
    }
    ...
    public function testCanThrowIgnoreEventIfWeHaveTaxInDeliveryPointInvoiceButWithoutAmount()
    {
        ...
        $tax = new InvoiceTax();
        $tax->setType(InvoiceTax::TYPE_TAX_CSPE);
        ...
        $analyzer = $this->getAnalyzerMock(CspeTotalTaxExcludedAnalyzer::class, [$taxService, $consumptionService, $conversionService]);
        $analyzer->expects($this->once())->method('ignore')->with(transInfo('ht_amount_missing', ['type' => 'tax', 'tax' => strtoupper(InvoiceTax::TYPE_TAX_CSPE)]), 'tax.cspe.total');
    ...
    public function testCanThrowAnomalyIfDataDoesNotMatch()
    {
        ...
        $analyzer->expects($this->once())->method('anomaly')->with(
            ...
            transInfo('amount_incorrect', [
                'amount_type' => 'HT',
                'type' => 'tax',
                'tax' => strtoupper(InvoiceTax::TYPE_TAX_CSPE)
            ]),
            ...
            'tax.cspe.total', new AmountDiff(10**5, 11.11, Anomaly::PROFIT_PROVIDER)
        );
        $analyzer->analyze($deliveryPointInvoice);
    }
