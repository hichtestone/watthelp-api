...
class ListController
{
    ...
    /**
     * @Route("/invoice/analysis", methods={"GET"})
     * @ConstraintValidator(class="App\Request\Validator\Invoice\Analysis\FilterConstraintList", options={"type"="App\Entity\Invoice\Analysis"})
     ...
     * @SWG\Parameter(name="sort", in="query", required=false, type="string", enum={"id","invoices","status", "created_at"})
     ...
     *      @SWG\Property(property="data", type="array", @SWG\Items(type="object", ref=@Model(type=App\Entity\Invoice\Analysis::class, groups={"default"}))),
     *     ),
     *     description=""
     * )
    ...
    public function __invoke(Request $request, Pagination $pagination, UserInterface $connectedUser): JsonResponse
    {
        ...
        $data = $this->analysisManager->findByFilters($connectedUser->getClient(), $filters, $pagination);
